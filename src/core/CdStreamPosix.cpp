...
static int sem_post(sem_t *sem)
{
	return LWP_SemPost(*sem) == LWP_SUCCESSFUL ? 0 : -1;
}

static int sem_wait(sem_t *sem)
{
	return LWP_SemWait(*sem) == LWP_SUCCESSFUL ? 0 : -1;
}

static int wii_thread_create(lwp_t *thread, const void *attr, void *(*start_routine) (void *), void *arg)
{
	return LWP_CreateThread(thread, start_routine, arg, NULL, 0x8000, 64) == LWP_SUCCESSFUL ? 0 : EINVAL;
}
...
#elif defined(__WII__)
			status = wii_thread_create(&gpReadInfo[i].pChannelThread, NULL, CdStreamThread, (void*)channelI);
#else
...
#elif defined(__WII__)
	status = wii_thread_create(&_gCdStreamThread, NULL, CdStreamThread, nil);
#else
...
#ifdef __WII__
		LWP_JoinThread(gpReadInfo[i].pChannelThread, nil);
#elif !defined(__WIIU__)
