...
enum Config {
	...
	// Pool sizes
#ifdef __WII__
	NUMPTRNODES = 26000,  // PS2 value, weaker CPU can't simulate as many active entities
	NUMENTRYINFOS = 3200,
	NUMPEDS = 90,
	NUMVEHICLES = 70,
#else
...
#		if !defined(__WIIU__) && !defined(__WII__) // not supported in gx2 librw / not supported by the fixed-function GX backend
...
// Streaming
	...
	#ifndef __WII__ // libogc's LWP has no thread-cancellation equivalent to interrupt a blocked read
...
#ifdef __WII__
// no libsndfile portlib for devkitPPC/libogc
#undef AUDIO_OAL_USE_SNDFILE
