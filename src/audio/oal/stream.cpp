...
#define CLOSE_ON_ERROR(op)\
		...
		// these multi-char constants are reversed to match a raw fread on little-endian hosts
#ifdef BIGENDIAN
#define RIFF_ID 'RIFF'
#define WAVE_ID 'WAVE'
#define FMT_ID  'fmt '
...
#define FMT_ID  ' tmf'
		...
		CLOSE_ON_ERROR(DataHeader.ID != RIFF_ID);

		// TODO? validate filesizes

		int WAVE;
		CLOSE_ON_ERROR(fread(&WAVE, 4, 1, m_pFile) == 0);
		CLOSE_ON_ERROR(WAVE != WAVE_ID)
		CLOSE_ON_ERROR(fread(&DataHeader, sizeof(DataHeader), 1, m_pFile) == 0);
		CLOSE_ON_ERROR(DataHeader.ID != FMT_ID);
		memLittle32(&DataHeader.Size);
		...
		memLittle16(&m_FormatHeader.AudioFormat);
		memLittle16(&m_FormatHeader.NumChannels);
		memLittle32(&m_FormatHeader.SampleRate);
		memLittle32(&m_FormatHeader.ByteRate);
		memLittle16(&m_FormatHeader.BlockAlign);
		memLittle16(&m_FormatHeader.BitsPerSample);
