...
char* casepath(char const* path, bool checkPathFirst)
{
    ...
    #if defined(__SWITCH__) || defined(PSP2) || defined(__WII__)
    if( (c = strstr(p, ":/")) != NULL) // scheme used by some environments, eg. switch, vita, wii (sd:/...)
    {
        size_t deviceNameOffset = c - p + 3;
#if defined(__WII__)
        char deviceNamePath[deviceNameOffset + 1];
#else
        ...
        p = c + 1;
#if defined(__WII__)
        rl = strlen(deviceNamePath);
        strcpy(out, deviceNamePath);
        if(rl > 0 && out[rl-1] == '/')
            rl--;
#endif
    }
