...
#define AL_ONCE_FLAG_INIT 0
#elif defined(__WII__)
typedef lwp_t althrd_t;
typedef mutex_t almtx_t;
typedef cond_t alcnd_t;
typedef sem_t alsem_t;
typedef __wii_key_t altss_t;
typedef uint32_t alonce_flag;
...
inline int almtx_lock(almtx_t *mtx)
{
    return LWP_MutexLock(*mtx) == 0 ? althrd_success : althrd_error;
}

inline int almtx_unlock(almtx_t *mtx)
{
    return LWP_MutexUnlock(*mtx) == 0 ? althrd_success : althrd_error;
}

inline int almtx_trylock(almtx_t *mtx)
{
    return LWP_MutexTryLock(*mtx) == 0 ? althrd_success : althrd_busy;
}

inline void *altss_get(altss_t tss_id)
{
    return wii_getspecific(tss_id);
}

inline int altss_set(altss_t tss_id, void *val)
{
    return wii_setspecific(tss_id, val);
}
...
inline void alcall_once(alonce_flag *once, void (*callback)(void))
{
    ...
    {
        callback();
        __sync_bool_compare_and_swap(once, 1, 2);
    }
