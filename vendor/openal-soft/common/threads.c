...
int althrd_create(althrd_t *thr, althrd_start_t func, void *arg)
{
    ...
    if (LWP_CreateThread(thr, althrd_starter, cntr, NULL, THREAD_STACK_SIZE, 80) != 0)
    {
        free(cntr);
        return althrd_error;
    }
...
{
    (void)thr;
    /* LWP threads are joined via LWP_JoinThread; there's no separate detach,
     * so this is only ever meaningful paired with althrd_join elsewhere. */
    return althrd_success;
}
...
{
    void *retval = NULL;
    int ret = LWP_JoinThread(thr, &retval);
    if(res) *res = (int)(intptr_t)retval;
    return ret == 0 ? althrd_success : althrd_error;
}

int almtx_init(almtx_t *mtx, int type)
{
    return LWP_MutexInit(mtx, type == almtx_recursive) == 0 ? althrd_success : althrd_error;
}
...
int alcnd_init(alcnd_t *cond)
{
    return LWP_CondInit(cond) == 0 ? althrd_success : althrd_error;
}

int alcnd_signal(alcnd_t *cond)
{
    return LWP_CondSignal(*cond) == 0 ? althrd_success : althrd_error;
}

int alcnd_broadcast(alcnd_t *cond)
{
    return LWP_CondBroadcast(*cond) == 0 ? althrd_success : althrd_error;
}

int alcnd_wait(alcnd_t *cond, almtx_t *mtx)
{
    return LWP_CondWait(*cond, *mtx) == 0 ? althrd_success : althrd_error;
}
...
int alsem_init(alsem_t *sem, unsigned int initial)
{
    return LWP_SemInit(sem, initial, 255) == 0 ? althrd_success : althrd_error;
}
...
int alsem_post(alsem_t *sem)
{
    return LWP_SemPost(*sem) == 0 ? althrd_success : althrd_error;
}

int alsem_wait(alsem_t *sem)
{
    return LWP_SemWait(*sem) == 0 ? althrd_success : althrd_error;
}

int alsem_trywait(alsem_t *sem)
{
    u32 value = 0;
    if (LWP_SemGetValue(*sem, &value) != 0)
        return althrd_error;
    if (value == 0)
        return althrd_busy;
    return LWP_SemWait(*sem) == 0 ? althrd_success : althrd_error;
...
int altss_create(altss_t *tss_id, altss_dtor_t callback)
{
    return wii_key_create(tss_id, callback) == 0 ? althrd_success : althrd_error;
}

void altss_delete(altss_t tss_id)
{
    wii_key_delete(tss_id);
}
