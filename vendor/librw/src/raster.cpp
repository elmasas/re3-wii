...
// DXT1 and CMPR hold the same blocks, so this skips a decompress and a recompress
static rw::Raster*
d3d_to_gx(rw::Raster *ras)
{
#ifdef __WII__
	using namespace rw;
	...
	Raster *newras = Raster::create(ras->width, ras->height, ras->depth,
		ras->format | Raster::TEXTURE | Raster::DONTALLOCATE);
	uint32 size = ((ras->width+3)/4) * ((ras->height+3)/4) * 8;
	uint8 *srcpx = ras->lock(0, Raster::LOCKREAD);
	bool32 ok = srcpx && gx::rasterFromDxt1(newras, srcpx, size, d3dras->hasAlpha);
	ras->unlock(0);
	if(!ok){
		newras->destroy();
		return nil;
	...
	if((ras->platform == PLATFORM_D3D8 || ras->platform == PLATFORM_D3D9) && rw::platform == PLATFORM_GL3){
		...
		if(newras){
			...
			return newras;
	         ...
	         rw::platform == PLATFORM_GX){
		Raster *newras = d3d_to_gx(ras);
		if(newras){
			ras->destroy();
			return newras;
		}
