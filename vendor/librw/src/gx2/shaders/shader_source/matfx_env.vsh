...
; $SPI_VS_OUT_CONFIG.VS_EXPORT_COUNT = 4
; $NUM_SPI_VS_OUT_ID = 2
; color
...
; C20
; $UNIFORM_VARS[8].name = "u_lightParams"
; $UNIFORM_VARS[8].type = "vec4"
; $UNIFORM_VARS[8].count = 8
; $UNIFORM_VARS[8].block = -1
; $UNIFORM_VARS[8].offset = 80
; C28
; $UNIFORM_VARS[9].name = "u_lightDirection"
; $UNIFORM_VARS[9].type = "vec4"
; $UNIFORM_VARS[9].count = 8
; $UNIFORM_VARS[9].block = -1
; $UNIFORM_VARS[9].offset = 112
; C36
; $UNIFORM_VARS[10].name = "u_lightColor"
; $UNIFORM_VARS[10].type = "vec4"
; $UNIFORM_VARS[10].count = 8
; $UNIFORM_VARS[10].block = -1
; $UNIFORM_VARS[10].offset = 144
; C44
; $UNIFORM_VARS[11].name = "u_colorClamp"
; $UNIFORM_VARS[11].type = "vec4"
; $UNIFORM_VARS[11].count = 1
; $UNIFORM_VARS[11].block = -1
; $UNIFORM_VARS[11].offset = 176
; C45
; $UNIFORM_VARS[12].name = "u_envColor"
; $UNIFORM_VARS[12].type = "vec4"
; $UNIFORM_VARS[12].count = 1
; $UNIFORM_VARS[12].block = -1
; $UNIFORM_VARS[12].offset = 180
...
03 LOOP_START_DX10 FAIL_JUMP_ADDR(8)
    04 ALU_BREAK: ADDR(160) CNT(3)
        53 y: SETGT_INT       R0.y, 0x8, R6.w
        54 x: PRED_SETNE_INT  ____,  R0.y, 0.0f UPDATE_EXEC_MASK UPDATE_PRED
    05 ALU_BREAK: ADDR(163) CNT(4)
        55 t: INT_TO_FLT      R0.x, R6.w
        56 x: MOVA_FLOOR      ____, PS0
        57 w: SETE_DX10       R0.w, C20[AR.x].x, 0.0f
        58 x: PRED_SETE_INT   ____, R0.w, 0.0f UPDATE_EXEC_MASK UPDATE_PRED 
    06 ALU: ADDR(167) CNT(23)
        59 x: MOVA_FLOOR  ____, R0.x
           w: MOV         R0.w, 0.0f
           t: ADD_INT     R6.w, R6.w, 0x1
        60 x: MOV         ____, C28[AR.x].x
           y: MOV         ____, C28[AR.x].y
           z: MOV         ____, C28[AR.x].z
        61 x: DOT4        ____, R6.x, -PV0.x
           y: DOT4        ____, R6.y, -PV0.y
           z: DOT4        ____, R6.z, -PV0.z
           w: DOT4        ____, R6.z, -R0.w
           t: SETE_DX10   R7.w, C20[AR.x].x, 1.0f
        62 w: MAX         ____, PV0.x, 0.0f
        63 x: MUL         ____, PV0.w, C36[AR.x].x
           y: MUL         ____, PV0.w, C36[AR.x].y
           z: MUL         ____, PV0.w, C36[AR.x].z
        64 x: ADD         ____, PV0.x, R7.x
           y: ADD         ____, PV0.y, R7.y
           z: ADD         ____, PV0.z, R7.z
        65 x: CNDE_INT    R7.x, R7.w, R7.x, PV0.x
           y: CNDE_INT    R7.y, R7.w, R7.y, PV0.y
           z: CNDE_INT    R7.z, R7.w, R7.z, PV0.z
07 LOOP_END CF_CONST(0) PASS_JUMP_ADDR(4)
08 ALU: ADDR(190) CNT(15)
    66 x: MULADD    R3.x,   R7.x,  C14.z, R3.x CLAMP
       y: MULADD    R3.y,   R7.y,  C14.z, R3.y CLAMP
       z: MULADD    R3.z,   R7.z,  C14.z, R3.z CLAMP
    67 x: MAX       ____,   R3.x,  C44.x
       y: MAX       ____,   R3.y,  C44.y
       z: MAX       ____,   R3.z,  C44.z
       w: MAX       ____,   R3.w,  C44.w
    68 x: MUL       R6.x,   PV0.x, C45.x
       y: MUL       R6.y,   PV0.y, C45.y
       z: MUL       R6.z,   PV0.z, C45.z
       w: MUL       R6.w,   PV0.w, C45.w
    69 x: MUL       R3.x,   R3.x,  C13.x
...
12 EXP: PARAM2, R9 NO_BARRIER
13 EXP: PARAM3, R8 NO_BARRIER
14 EXP_DONE: PARAM4, R6 NO_BARRIER
END_OF_PROGRAM
